...
(function(jQuery)
{
  ...
  jQuery.railsAutocomplete.fn.extend({
    init: function(e) {
      ...
      var autocomplete_factory = jQuery(e).attr('data-autocomplete-factory') || 'autocomplete';
      e.min_length = jQuery(e).attr('min-length') || 2;
      e.append_to = jQuery(e).attr('data-append-to') || null;
      e.no_data_label = jQuery(e).attr('data-no-data-label') || 'no existing match';
      ...
      jQuery(e)[autocomplete_factory]({
        ...
        source: function( request, response ) {
          ...
          jQuery.getJSON( jQuery(e).attr('data-autocomplete'), params, function() {
            if(arguments[0].length == 0) {
              arguments[0] = []
              arguments[0][0] = { id: "", label: e.no_data_label, dataNotFound: true }
            }
        ...
        select: function( event, ui ) {
          if (ui.item.dataNotFound) {
            jQuery(e).trigger('railsAutocomplete.dataNotFound', ui);
            return false;
          }
